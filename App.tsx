...
const ProfileView: React.FC<{ user: User; profile: UserProfile, wallets: Wallet[]; onLogout: () => void; onAddWallet: () => void; onUpdateProfile: (updates: Partial<UserProfile>) => Promise<void>; onUpdateUser: (updates: Partial<User>) => void; }> = ({ user, profile, wallets, onLogout, onAddWallet, onUpdateProfile, onUpdateUser }) => {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-light-surface dark:bg-dark-surface p-4 rounded-2xl shadow-sm">
                ...
                <div className="space-y-2">
                    {wallets.map(w => (
                        <div key={w.id} className="flex justify-between items-center p-2 rounded-lg">
                            <span>{w.name}</span>
...
const AppContent: FC<AppContentProps> = ({ user, profile, transactions, goals, wallets, debts, onLogout, onUpdateProfile, onUpdateUser }) => {
    ...
    const renderContent = () => {
        ...
        switch (activeNav) {
            ...
            case 'profile': return <ProfileView user={user} profile={profile} wallets={wallets} onLogout={onLogout} onAddWallet={() => setWalletModalOpen(true)} onUpdateProfile={onUpdateProfile} onUpdateUser={onUpdateUser} />;
